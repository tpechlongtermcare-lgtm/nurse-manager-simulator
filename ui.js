...
export class GameUI {
    ...
    this.root.innerHTML = `
      <main class="game">
        <section class="top-sheet">
          <div class="day-line">
            <div class="day-label">第 ${state.day} 天 / 30</div>
        ...
        <div class="game-main">
          <div class="status-strip">
            <span>在職 ${activeCount} / ${state.staff.length}　${state.staffShortage ? "・人力缺口中" : ""}</span>
            <span>
  ...
  renderMeter(key, value) {
    return `
      <div class="meter-row" data-meter-row="${key}">
        ...
        <div class="meter-track"><div class="meter-fill" data-meter="${key}" style="width:${value}%"></div></div>
  ...
  showDawn(state, quitters = [], onContinue) {
    ...
    overlay.innerHTML = `
      <section class="overlay-paper dawn-paper">
        <div class="eyebrow">晨間交班</div>
    ...
    overlay.querySelector("#dawnContinue").addEventListener("click", () => {
      overlay.remove();
      onContinue();
    });
  }
        ...
        <div class="staff-list">
          ${state.staff.map(npc => `
            <div class="staff-row ${npc.quit ? "quit" : ""}">
              <div class="staff-top"><strong>${esc(npc.name)}</strong><span class="staff-role">${esc(npc.role)}・${esc(npc.trait)}</span></div>
  ...
  showAchievements() {
    ...
    overlay.className = "overlay";
    overlay.innerHTML = `
      <section class="overlay-paper achievement-paper">
        <h2>成就總覽　${unlocked.size} / ${this.data.achievements.length}</h2>
